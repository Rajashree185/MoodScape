...
const TRACK_CATEGORIES = [
  {
    ...
    tracks: [
      ...
      { id:"thunder",icon:"⛈️", label:"Deep Thunder",     ytId:"AIjwrIdtlRg", ambient:"rain"   },
      { id:"snow",   icon:"❄️", label:"Snowy Night",      ytId:"V37Q3l6AFAU", ambient:"creek"  },
    ],
  },
  {
    name: "Ambience",
    tracks: [
      ...
      { id:"night",  icon:"🌙", label:"Night Crickets",   ytId:"7GoXVDH656g", ambient:"forest" },
      { id:"cafe",   icon:"☕", label:"Cafe Ambience",    ytId:"Mckcmh-OU5M", ambient:"creek"  },
    ],
  },
  {
    ...
    tracks: [
      ...
      { id:"piano",  icon:"🎹", label:"Calm Piano",       ytId:"gF-FFtv1qE4", ambient:"lofi"   },
      { id:"space",  icon:"🌌", label:"Deep Space",       ytId:"QiFXQwMg8CQ", ambient:"lofi"   },
    ],
  },
];

const ALL_TRACKS = TRACK_CATEGORIES.flatMap(c => c.tracks);
...
function SoothingMusic({ onAmbientChange, onPlayingChange, onTrackChange }) {
  ...
  const [activeTrack, setActive]  = useState(ALL_TRACKS[0]);
  ...
  const switchTrack = (track) => {
    ...
    try {
      ...
      setErrorTracks(prev => ({ ...prev, [track.id]: true }));
    }
  };
  ...
  const isActive = (trackId) => activeTrack.id === trackId;

  const renderTrackButton = (tr) => {
    const active = isActive(tr.id);
    return (
    <button key={tr.id} onClick={() => switchTrack(tr)}
      className={active && playing ? `ambient-track-active ${tr.ambient}` : ""}
      style={{
        ...
        transition:"all 0.25s", fontSize:"0.75rem", fontFamily:"inherit",
        fontWeight: active ? "700" : "400",
        opacity: errorTracks[tr.id] ? 0.45 : 1,
      }}>
      <span style={{
        fontSize:"1.5rem",
        filter: active && playing ? `drop-shadow(0 0 8px ${AMBIENT_CONFIG[tr.ambient]?.color})` : "none",
        transition:"filter 0.5s",
        position:"relative",
      }}>
        {errorTracks[tr.id] ? "⚠️" : tr.icon}
        {active && !playing && (
          <span style={{
            position:"absolute", bottom:"-2px", right:"-6px",
            fontSize:"0.55rem", background:"var(--bg-card)", borderRadius:"50%",
  ...
  return (
    <div className="glass-panel neu-card" style={{padding:"26px", borderRadius:"18px"}}>
      ...
      <div style={{display:"flex", alignItems:"center", gap:"10px", marginBottom:"20px", flexWrap:"wrap"}}>
        <div style={{fontSize:"0.72rem", textTransform:"uppercase", letterSpacing:"0.1em", color:"var(--text-muted)", display:"flex", alignItems:"center", gap:"8px"}}>
          🎵 Soothing Music
          {playing && (
            <span style={{ display:"flex", gap:"2px", alignItems:"flex-end", height:"14px" }}>
              {[1,2,3,4,5].map(i => (
                <span key={i} style={{
                  width:"3px", background: activeTrack ? AMBIENT_CONFIG[activeTrack.ambient]?.color || "#22d3a8" : "#22d3a8",
                  borderRadius:"2px", height:`${6 + i * 3}px`,
        ...
        {status === "loading" && <span style={{fontSize:"0.7rem", color:"var(--text-muted)"}}>Loading player…</span>}
        {status === "error" && initAttempted && (
          <button onClick={retryPlayer} style={{
            fontSize:"0.7rem", color:"#fca5a5", background:"none", border:"1px solid rgba(248,113,113,0.2)",
            borderRadius:"6px", padding:"3px 10px", cursor:"pointer", fontFamily:"inherit",
          }}>
            ↻ Retry
          </button>
        )}
      ...
      {/* Categorized track grids */}
      {TRACK_CATEGORIES.map((cat, ci) => (
        <div key={cat.name} style={{marginBottom: ci < TRACK_CATEGORIES.length - 1 ? "18px" : "22px"}}>
          <div style={{
            fontSize:"0.68rem", textTransform:"uppercase", letterSpacing:"0.1em",
            color:"var(--text-muted)", marginBottom:"10px", fontWeight:"600",
            padding:"0 4px", display:"flex", alignItems:"center", gap:"6px",
          }}>
            <span style={{width:"12px", height:"2px", borderRadius:"2px", background:"var(--accent-mid)"}}/>
            {cat.name}
          </div>
          <div style={{display:"grid", gridTemplateColumns:"repeat(3, 1fr)", gap:"8px"}}>
            {cat.tracks.map(renderTrackButton)}
          </div>
        </div>
      ))}
      ...
      <div style={{display:"flex", alignItems:"center", gap:"12px", marginBottom:"20px"}}>
        <span style={{fontSize:"0.85rem", color:"var(--text-muted)"}}>{getVolumeIcon()}</span>
        <input type="range" min="0" max="100" step="5" value={volume}
          onChange={e => changeVolume(Number(e.target.value))}
          style={{flex:1, accentColor:"#22d3a8", cursor:"pointer"}}/>
        <span style={{fontSize:"0.72rem", color:"var(--text-muted)", minWidth:"28px", textAlign:"right"}}>
          {volume}%
        </span>
      </div>
      ...
      {ready && (
        <button onClick={togglePlay}
          style={{
            ...
            background: playing
              ? `linear-gradient(135deg, ${AMBIENT_CONFIG[activeTrack.ambient]?.color || '#3b82f6'}22, transparent)`
              : "linear-gradient(135deg, var(--accent-blue), #7c3aed)",
            color: playing ? (AMBIENT_CONFIG[activeTrack.ambient]?.color || "#fca5a5") : "#fff",
            border: playing ? `1px solid ${AMBIENT_CONFIG[activeTrack.ambient]?.color || '#3b82f6'}55` : "none",
            transition:"all 0.3s",
            boxShadow: playing ? "none" : "0 4px 20px rgba(59,130,246,0.35)",
          }}>
          {playing ? <>⏸ Pause · {activeTrack.icon} {activeTrack.label}</> : <>▶ Play · {activeTrack.icon} {activeTrack.label}</>}
        </button>
      )}
      ...
      {!ready && (
        <div style={{
          ...
          {status === "loading" ? "⏳ Initializing YouTube player…" : "⚠ Click a track above to start"}
        </div>
      )}

      <div style={{marginTop:"12px", fontSize:"0.68rem", color:"var(--text-muted)", textAlign:"center", lineHeight:1.5}}>
        Streams from YouTube · Click a track to play · Volume: {volume}%
      </div>
    </div>
  );
}
          ...
          {activeTab === "coping" && (
            <div className="fade-in">
              <p style={{color:"var(--text-secondary)",fontSize:"0.875rem",marginBottom:"24px",lineHeight:1.6}}>
                Immerse yourself in curated ambient soundscapes to relax, focus, and recharge. 🌿
              </p>
